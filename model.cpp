...
*/
void VModel::loadModelFromFile(const std::string& path, const vulkan::sampler& texture_sampler, const vulkan::descriptorPool& descriptor_pool,
	const vulkan::descriptorSetLayout& material_descriptor_set_layout)
{
	...
	std::cout << vertex_buffer_size << " " << index_buffer_size;
	...
	for (const auto& group : groups)
	{
		...
		tempvertexsize += vertex_section_size;
		tempindexsize += index_section_size;
